...
public class CustomerRequestFilter {
    ...
    public static void doCustomerFilter(String fileUrl, HttpServletRequest req) {
        doFullFileName(fileUrl,req);
        doWatermarkTxt(fileUrl,req);
    }
    ...
    private static void doFullFileName(String fileUrl, HttpServletRequest req) {
        // 从url中提取filename
        String fullFileName = WebUtils.getUrlParameterReg(fileUrl, FILE_NAME_KEY);
        // 如果不存在从request中获取
    ...
    private static void doWatermarkTxt(String fileUrl, HttpServletRequest req) {
